...
class XMPPHP_XMPP extends XMPPHP_XMLStream {
	...
	public function subscribe($jid) {
		$this->send("<presence type='subscribe' to='{$jid}' />");
	...
	public function presence_handler($xml) {
		...
		if(array_key_exists('type', $xml->attrs) and $xml->attrs['type'] == 'subscribe') {
			if($this->auto_subscribe) {
				$this->send("<presence type='subscribed' to='{$xml->attrs['from']}'/>");
				$this->send("<presence type='subscribe' to='{$xml->attrs['from']}'/>");
			}
